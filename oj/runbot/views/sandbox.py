...
class SandBox:
    ...
    def create(self):
        ...
        if available_images:
            ...
            # not found, pull?
            print('No docker image found for %s, type `i` to pull ith image from remote server, or else quit' %
                  self.config['suffix'])
            for i, image in enumerate(available_images):
                print(i, image, 'Supported language: %s' % ','.join(compiler.images[image]), sep='\t')
            try:
                i = int(input())
    ...
    def run(self, input, keep_running=False):
        ...
        if not self.proc:
            ...
            wrapper_command = '~/wrapper -d {cwd} -t {tl} {stl} -m {ml} -o {ol} {command}'.format(
                ...
                stl='-k {tl}'.format(
                    tl=self.config['sub_time_limit'] * SandBox.CONFIG_TIME_RATIO) if keep_running else '',
    ...
    def _run_container(self, image):
        p = subprocess.run('docker run -td --rm %s /bin/bash' % image, capture_output=True, shell=True,
                           encoding='utf-8')
        if p.returncode != 0:
    ...
    def _copy_to_container(self, src, dst):
        p = subprocess.run('docker cp "%s" "%s:%s"' % (src, self.container, dst), capture_output=True, shell=True,
                           encoding='utf-8')
        if p.returncode != 0:
    ...
    def _update_container(self, cpu, memory):
        p = subprocess.run(
            'docker update --cpus="{cpu}" --memory="{memory}m" --memory-swap="{memory}m" {container}'
                .format(cpu=cpu,
                memory=memory,
